...
<form action="{{ route('admin.retentions.store') }}" method="POST">
    @csrf

    <div class="grid grid-cols-1 p-4 xl:grid-cols-3 xl:gap-4">
        <div class="mb-4 col-span-full xl:mb-2">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Crear Regla de Retención</h1>
            <p class="text-sm text-gray-500 mt-1">Define las bases y porcentajes de retención para un grupo de impuestos</p>
        </div>
                    ...
                    <div class="col-span-3">
                        ...
                        <select name="tax_group" id="tax_group" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            ...
                            @foreach($taxGroups as $value => $label)
                                <option value="{{ $value }}" @selected(old('tax_group') === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('tax_group')
                    ...
                    <div class="col-span-3">
                        ...
                        <select name="product_type" id="product_type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                            ...
                            @foreach($productTypes as $value => $label)
                                <option value="{{ $value }}" @selected(old('product_type') === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('product_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
