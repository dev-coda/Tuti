...
<div class="grid grid-cols-1 p-4 xl:grid-cols-3 xl:gap-4">
        ...
        <div class="mt-3 bg-white border border-gray-200 rounded-lg shadow-sm p-3" id="inventory-sync-progress" data-status-url="{{ route('settings.inventory-sync-status') }}">
            ...
            <div class="mt-2">
                ...
                @if(!empty($syncStatus['error_message']))
                    <p class="mt-2 text-sm text-red-700" data-sync-error>{{ $syncStatus['error_message'] }}</p>
                @else
                    <p class="mt-2 text-sm text-red-700 hidden" data-sync-error></p>
                @endif
            </div>
        </div>
    ...
    <div class="col-span-full mb-6">
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
            ...
            <form action="{{ route('settings.update-express-48h') }}" method="POST" id="express48hForm" class="space-y-4">
                @csrf
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <p class="mt-2 text-xs text-gray-500">Cuando está activado, los usuarios verán la opción de entrega express (Coordinadora 48h) en el carrito y el método <code class="text-xs bg-gray-100 px-1 rounded">express</code> queda habilitado en Métodos de envío. En producción puedes forzar que permanezca desactivada con la variable de entorno <code class="text-xs bg-gray-100 px-1 rounded">COORDINADORA_EXPRESS_48H_DISABLED=true</code> (tiene prioridad sobre este interruptor).</p>
                    @if(config('services.coordinadora.express_48h_disabled'))
                        <p class="mt-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">
                            Entrega Especial está forzada a <strong>apagada</strong> por <code class="text-xs bg-red-100 px-1 rounded">COORDINADORA_EXPRESS_48H_DISABLED=true</code> en el servidor. Quita o pon esa variable en <code class="text-xs bg-red-100 px-1 rounded">false</code> y ejecuta <code class="text-xs bg-red-100 px-1 rounded">php artisan config:cache</code> para que este interruptor tenga efecto.
                        </p>
                    @elseif(!\App\Models\Setting::isExpress48hEnabled())
                        <p class="mt-2 text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3">
                            Entrega Especial no se muestra en el carrito mientras este interruptor esté desactivado.
                        </p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           name="express_free_shipping_min"
                           min="0"
                           step="1"
                           value="{{ old('express_free_shipping_min', \App\Models\Setting::expressFreeShippingMinimum()) }}"
