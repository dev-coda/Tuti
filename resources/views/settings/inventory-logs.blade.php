        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="flex gap-3">
                    ...
                    <form action="{{ route('settings.sync-inventory') }}?sync=1" method="POST" class="inline">
                        @csrf
                        <button type="submit" 
                                onclick="this.innerHTML='<svg class=\'animate-spin w-4 h-4 mr-2\' fill=\'none\' viewBox=\'0 0 24 24\'><circle class=\'opacity-25\' cx=\'12\' cy=\'12\' r=\'10\' stroke=\'currentColor\' stroke-width=\'4\'></circle><path class=\'opacity-75\' fill=\'currentColor\' d=\'M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z\'></path></svg>Procesando...'; this.disabled=true;"
                                class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                            </svg>
                            Sincronizar (Sync - Espera)
                        </button>
                    </form>
                </div>
            </div>
            <p class="text-xs text-blue-500 mt-3">
                <strong>Async:</strong> Requiere que Horizon esté corriendo (<code>php artisan horizon</code>). 
                <strong>Sync:</strong> Ejecuta inmediatamente y espera a que termine (puede tardar varios minutos).
            </p>
        </div>
    </div>
</div>
