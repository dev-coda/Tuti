        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Crear Descuento por Volumen</h1>
                        <p class="text-gray-600">Configura un nuevo descuento basado en la cantidad comprada</p>
                    </div>
                ...
                <form action="{{ route('volume-discounts.store') }}" method="POST" class="space-y-6">
                    @csrf
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="discount_type" id="discount_type" required
                                    ...
                                    <option value="fixed_amount" {{ old('discount_type') == 'fixed_amount' ? 'selected' : '' }}>Valor Fijo ($)</option>
                                </select>
                                @error('discount_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="min_quantity" id="min_quantity" value="{{ old('min_quantity') }}" 
                                    min="1" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @error('min_quantity')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="number" name="max_quantity" id="max_quantity" value="{{ old('max_quantity') }}" 
                                    min="1"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="applies_to" id="applies_to" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option value="products" {{ old('applies_to') == 'products' ? 'selected' : '' }}>Productos Específicos</option>
                                <option value="categories" {{ old('applies_to') == 'categories' ? 'selected' : '' }}>Categorías</option>
                                <option value="brands" {{ old('applies_to') == 'brands' ? 'selected' : '' }}>Marcas</option>
                                <option value="vendors" {{ old('applies_to') == 'vendors' ? 'selected' : '' }}>Proveedores</option>
                                <option value="cart" {{ old('applies_to') == 'cart' ? 'selected' : '' }}>Carrito Completo</option>
                            </select>
                            @error('applies_to')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="applies_to_ids_section" class="hidden">
                            ...
                            <input type="hidden" name="applies_to_ids" id="applies_to_ids_input" value="">
