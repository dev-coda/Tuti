        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Editar Descuento por Volumen</h1>
                        <p class="text-gray-600">Modifica la configuración del descuento basado en cantidad</p>
                    </div>
                ...
                <form action="{{ route('volume-discounts.update', $volumeDiscount) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700">Nombre del Descuento</label>
                                <input type="text" name="name" id="name" value="{{ old('name', $volumeDiscount->name) }}" required
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $volumeDiscount->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <select name="discount_type" id="discount_type" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    <option value="percentage" {{ old('discount_type', $volumeDiscount->discount_type) == 'percentage' ? 'selected' : '' }}>Porcentaje (%)</option>
                                    <option value="fixed_amount" {{ old('discount_type', $volumeDiscount->discount_type) == 'fixed_amount' ? 'selected' : '' }}>Valor Fijo ($)</option>
                                </select>
                                @error('discount_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', $volumeDiscount->discount_value) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label for="min_quantity" class="block text-sm font-medium text-gray-700">Cantidad Mínima</label>
                                <input type="number" name="min_quantity" id="min_quantity" value="{{ old('min_quantity', $volumeDiscount->min_quantity) }}" 
                                    min="1" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @error('min_quantity')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                <label for="max_quantity" class="block text-sm font-medium text-gray-700">Cantidad Máxima (Opcional)</label>
                                <input type="number" name="max_quantity" id="max_quantity" value="{{ old('max_quantity', $volumeDiscount->max_quantity) }}" 
                                    min="1"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="valid_from" id="valid_from" value="{{ old('valid_from', $volumeDiscount->valid_from->format('Y-m-d\TH:i')) }}" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @error('valid_from')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="valid_to" id="valid_to" value="{{ old('valid_to', $volumeDiscount->valid_to->format('Y-m-d\TH:i')) }}" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @error('valid_to')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="applies_to" class="block text-sm font-medium text-gray-700">Aplicar a</label>
                            <select name="applies_to" id="applies_to" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option value="products" {{ old('applies_to', $volumeDiscount->applies_to) == 'products' ? 'selected' : '' }}>Productos Específicos</option>
                                <option value="categories" {{ old('applies_to', $volumeDiscount->applies_to) == 'categories' ? 'selected' : '' }}>Categorías</option>
                                <option value="brands" {{ old('applies_to', $volumeDiscount->applies_to) == 'brands' ? 'selected' : '' }}>Marcas</option>
                                <option value="vendors" {{ old('applies_to', $volumeDiscount->applies_to) == 'vendors' ? 'selected' : '' }}>Proveedores</option>
                                <option value="cart" {{ old('applies_to', $volumeDiscount->applies_to) == 'cart' ? 'selected' : '' }}>Carrito Completo</option>
                            </select>
                            @error('applies_to')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="applies_to_ids_section" class="hidden">
                            <label for="applies_to_ids" class="block text-sm font-medium text-gray-700">Seleccionar Elementos</label>
                            ...
                            <input type="hidden" name="applies_to_ids" id="applies_to_ids_input" value="{{ json_encode(old('applies_to_ids', $volumeDiscount->applies_to_ids ?? [])) }}">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="active" id="active" value="1" {{ old('active', $volumeDiscount->active) ? 'checked' : '' }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const appliesToIdsSection = document.getElementById('applies_to_ids_section');
    ...
    const appliesToIdsInput = document.getElementById('applies_to_ids_input');
    ...
    const existingIds = JSON.parse(appliesToIdsInput.value || '[]');
    ...
    appliesToSelect.addEventListener('change', function() {
        ...
        if (value === 'cart') {
            appliesToIdsSection.classList.add('hidden');
            return;
        }

        appliesToIdsSection.classList.remove('hidden');
        loadElements(value);
    });
    ...
    function updateHiddenInput() {
        ...
        appliesToIdsInput.value = JSON.stringify(ids);
    }
