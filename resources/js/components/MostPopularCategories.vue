...
export default {
    data() {
        return {
            ...
            loading: true,
            error: null,
            sectionTitle: "Categorías Más Populares", // Default title
        };
    },
    methods: {
        ...
        async fetchCategories() {
            ...
            this.error = null;
            try {
                const response = await fetch("/api/categories/most-popular");
                if (!response.ok) {
                    throw new Error("Error al cargar las categorías");
                }
                const data = await response.json();
                console.log("Most Popular Categories API Response:", data); // Debug log
